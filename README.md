...
> @Transactional ใช้กำหนดให้คำสั่งภายใน Method ทำงานอยู่ภายใต้ Transaction เดียวกัน หากทำงานสำเร็จทั้งหมดจะทำการ COMMIT แต่ถ้าเกิด Error จะทำการ ROLLBACK ข้อมูลกลับไปก่อนเริ่ม Transaction

2. เพราะเหตุใดการฝากเงินจึงควรใช้ Transaction?
>เพราะการฝากเงินมีการทำงานมากกว่าหนึ่งขั้นตอน ได้แก่
> - เพิ่มยอดเงินใน Account
> - บันทึกประวัติการฝากเงิน
>ทั้งสองขั้นตอนควรสำเร็จพร้อมกัน หากขั้นตอนหนึ่งเกิด Error อีกขั้นตอนก็ควรถูกยกเลิกด้วย เพื่อป้องกันข้อมูลใน Database ไม่สอดคล้องกัน


3. COMMIT และ ROLLBACK ต่างกันอย่างไร?
> COMMIT คือการยืนยันและบันทึกการเปลี่ยนแปลงลง Database อย่างถาวร
> ROLLBACK คือการยกเลิกการเปลี่ยนแปลงที่เกิดขึ้นใน Transaction และย้อนกลับไปยังสถานะก่อนเริ่ม Transaction


4. Account และ DepositTransaction มีความสัมพันธ์แบบใด?
> เป็นความสัมพันธ์แบบ One-to-Many คือ Account 1 บัญชี สามารถมี DepositTransaction ได้หลายรายการ ดังนั้น DepositTransaction จึงมีความสัมพันธ์แบบ @ManyToOne กลับไปยัง Account

5. เมื่อเกิด Error ขณะใช้ `@Transactional` ข้อมูลใน Database เปลี่ยนแปลงอย่างไร?
> ข้อมูลที่เกิดขึ้นภายใน Transaction จะถูก ROLLBACK ทั้งหมด จากการทดลองใน Lab เมื่อฝากเงิน 1000 บาทแล้วเกิด Test Rollback
> - ยอด Account ยังคง 1000 บาท
> - ประวัติการฝากยังคงมีเพียง 1 รายการ
> - รายการที่กำลังทำอยู่ถูกยกเลิก
>ดังนั้น Database จะไม่เก็บข้อมูลที่เกิดขึ้นจาก Transaction ที่ล้มเหลว


6. จากการทดลอง เมื่อลบ `@Transactional` ออก ผลลัพธ์แตกต่างจากเดิมอย่างไร?
> เมื่อไม่มี @Transactional แล้วเกิด Error ระบบจะไม่สามารถย้อนกลับการเปลี่ยนแปลงทั้งหมดได้เหมือนเดิม ทำให้ข้อมูลที่บันทึกสำเร็จก่อนเกิด Error ยังคงอยู่ใน Database
> จากการทดสอบพบว่า deposit_transaction มี 2 รายการ แม้ Request ครั้งที่สองจะตอบ 500 Internal Server Error
> มี @Transactional → Error → ROLLBACK ทั้งหมด
> ไม่มี @Transactional → Error → ข้อมูลบางส่วนยังถูกบันทึก
